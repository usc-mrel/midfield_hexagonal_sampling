...
if figureflags.show_SOS_images == true   
    ...
    for i=1:Nimages
        ...
        elseif figureflags.show_error_images == true
            if i==5
                image = SEMAC_Images(:,:,figureflags.slice_num,4)-SEMAC_Images(:,:,figureflags.slice_num,1);
                image = image/max_value;
            elseif i==6
                image = SEMAC_Images(:,:,figureflags.slice_num,4)-SEMAC_Images(:,:,figureflags.slice_num,2);
                image = image/max_value;
            end
        end
        ...
        if i>=5
            imshow(image,[-max(max(image)) max(max(image))]);
        else
