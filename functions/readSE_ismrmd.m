...
%     info{nF}.ESP        = header.sequenceParameters.echo_spacing; % [msec]
    ...
    info{nF}.rawData.nr_phase_encodings = single(max(temp_raw_data.head.idx.kspace_encode_step_1)) + 1; % phase encoding index ; nr_interleaves for spiral imaging
    ...
    %     else
    %         kdata{nF} = data;
    %     endfunction [c_img, kdata, noise, info] = readSE_ismrmd(flags)
    ...
    if flags.CropPhaseEncoding
        if  header.encoding.encodingLimits.kspace_encoding_step_1.maximum >= header.encoding.reconSpace.matrixSize.y
            ...
            kdata{nF} = data(:, [remain_start : remain_end], :, :, :, :, :, :, :, :, :);
        end
    end
    ...
    if flags.Squeeze
        ...
        kdata{nF} = reshape(kdata{nF},[info{nF}.rawData.nr_samples, info{nF}.rawData.nr_phase_encodings, info{nF}.rawData.nr_slices, info{nF}.rawData.nr_channels ]);
