function mask = generateMask_old(Ny,Nz,mask_type,opt)
addReplicas = 0;
...
if mask_type == 4
    ...
    seg = 0;
    mask(:,Nz/2:Nz/2+1) = 1;
    mask(Ny/4+1+seg:Ny*3/4-seg,3:Nz-2) = 1;
...
if mask_type == 5
    ...
    if opt == 1
        ...
        f1 = figure();
        imshow((mask_showed),[]); xlabel("y",'FontSize', 20); ylabel("f",'FontSize', 20);
        title("Reduction in y",'FontSize', 20); set(f1,'color','w');
        hold on; line([260 260], [241 480],'color','k');
        hold on; line([520 520], [241 480],'color','k');
        hold on; line([260 520], [241 241],'color','k');
        hold on; line([260 520], [480 480],'color','k');
        total = sum(mask,'all');
        disp("Reduction in y-direction: "+total*100/Ny/Nz+"%");
    end
end
