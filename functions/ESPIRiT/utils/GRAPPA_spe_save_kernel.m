function [res,LIST,KEY] = GRAPPA_spe_save_kernel(kData,kCalib,kSize,lambda, dispp)
% res = GRAPPA(kData,kCalib,kSize,lambda [, disp)
%
% Modified from original GRAPPA.m by applying the same AtA on spectral
% encoding dimension (spe) for same slice and coil  ---Bochao Li 
%
...
%
% Inputs: 
%       kData     -   [Size x, Size y, Size sp, num coils] 2D multi-coil k-space data to reconstruct from.
...
pe = size(kData,2); fe = size(kData,1); spe = size(kData,3); coils = size(kData,4); % get sizes

res = kData*0;
MaxListLen = 100;
[sx,sy,Nz,nCoil] = size(kData);
LIST = zeros(kSize(1)*kSize(2)*nCoil,MaxListLen,size(res,3),size(res,4));
KEY =  zeros(kSize(1)*kSize(2)*nCoil,MaxListLen,size(res,3),size(res,4));
%[AtA] = corrMatrix(kCalib,kSize); % build coil correlation matrix
%AtA = dat2AtA(kCalib, kSize); % build coil calibrating matrix

for nspe = 1:spe % loop over spectral encoding dimension --BLi
    AtA = dat2AtA(kCalib, kSize); % build coil calibrating matrix
    for n=1:coils
        coil_time = tic;
        fprintf('reconstructing spe:%d, coil %d.  ',nspe,n);
        [res(:,:,nspe,n),LIST(:,:,nspe,n),KEY(:,:,nspe,n)] = ARC(squeeze(kData(:,:,nspe,:)), AtA, kSize, n, lambda); % reconstruct single coil image
        if dispp ~=0
...
function [res,LIST,KEY] = ARC(kData, AtA, kSize, c,lambda)
