function res = GRAPPA_spe_with_kernel(kData,kCalib,kSize,lambda, LIST, KEY)
...
for nspe = 1:spe % loop over spectral encoding dimension --BLi
    ...
    for n=1:coils
        ...
        list = LIST(:,:,nspe,n);
        key = KEY(:,:,nspe,n);
        res(:,:,nspe,n) = ARC(squeeze(kData(:,:,nspe,:)), AtA, kSize, n, lambda, list, key); % reconstruct single coil image
        if dispp ~=0
...
function [res] = ARC(kData, AtA, kSize, c,lambda, LIST, KEY);
